        ...
        {data.map((item, index) => (
          <tr key={index}>
            {columns.map((col, colIndex) => {
              // Create a proper cell structure that matches what the component expects
              const cellValue = col.accessorFn ? col.accessorFn(item) : item[col.id];
              const cellStructure = {
                cell: {
                  getValue: () => cellValue,
                  column: { Header: "" }, // Provide the column structure expected by statusCell
                },
                row: { original: item },
              };

              return <td key={colIndex}>{col.cell ? col.cell(cellStructure) : cellValue}</td>;
            })}
          </tr>
        ))}
      </tbody>
...
// Mock CLIEquivalent component
vi.mock("../../src/utils/uiutil", async () => {
  const actual = await vi.importActual("../../src/utils/uiutil");
  return {
    ...actual,
    // eslint-disable-next-line react/prop-types
    CLIEquivalent: ({ command }) => <div data-testid="cli-equivalent">CLI: {command}</div>,
  };
});
...
describe("Snapshots component", () => {
  ...
  test("displays CLI equivalent command", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText("CLI: snapshot list")).toBeInTheDocument();
    });
